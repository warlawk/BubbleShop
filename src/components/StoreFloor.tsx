...
/**
 * Generates a demand badge based on price vs retail value
 * @param price - Current selling price
 * @param retail - Base retail value
 * @returns Badge label and CSS classes indicating demand level
 */
...
/**
 * ShelfCard - Displays a single shelf with product info, stock bar, and controls
 * 
 * Shows product name, stock level, price stepper, and restock/swap buttons.
 * Highlights demand level with color-coded badges and event boosts.
 */
function ShelfCard({ s, shelf, dispatch, onPick }: {
  s: GameState; shelf: Shelf; dispatch: Dispatch<Action>; onPick: (id: number) => void;
}) {
  /** Maximum stock capacity for current upgrade level */
  const cap = shelfCapacity(s.capLvl);
  /** Product definition for this shelf's item */
  const item = shelf.itemId ? itemById(shelf.itemId) : null;
  /** Current selling price for this product */
  const price = shelf.itemId ? s.prices[shelf.itemId] : 0;
  /** Units available in back room storage */
  const storage = shelf.itemId ? s.storage[shelf.itemId] ?? 0 : 0;
  /** Demand badge based on price fairness */
  const badge = item ? demandBadge(price, item.retail) : null;
  /** Demand multiplier (1.0 = normal, >1 = high demand) */
  const demand = item ? demandFactor(price, item.retail) : 0;
  /** Event-driven demand boost multiplier */
  ...
  return (
    <div className="anim-pop bg-white border-[3px] border-ink rounded-[22px] overflow-hidden shadow-[0_6px_0_rgba(27,42,94,.22)] flex flex-col">
      {/* Decorative awning - colored by product or default blue */}
      <div className={`h-4 ${item ? "awning" : "awning awning-blue"}`} />
      <div className="p-2.5 flex flex-col gap-1.5 flex-1">
        {/* Header: product chip + demand badge */}
        ...
        {/* stock bar - visual indicator of current vs max capacity */}
        <div>
        ...
        {item ? (
          <>
            {/* price stepper - adjust price in $0.25 increments */}
        ...
        {/* Action buttons: restock from storage + swap product */}
...
/**
 * StoreFloor - Main store management view
 * 
 * Displays all shelves in a grid, allows:
 * - Placing new shelves in empty slots
 * - Leasing additional floor space
 * - Assigning products to shelves
 * - Adjusting prices and restocking
 * - Auto-placing products on empty shelves
 */
export function StoreFloor({ s, dispatch }: { s: GameState; dispatch: Dispatch<Action> }) {
  /** ID of shelf being assigned a product (null = no modal open) */
  const [pickFor, setPickFor] = useState<number | null>(null);
  /** Total units currently on all shelves */
  const totalStock = s.shelves.reduce((a, sh) => a + sh.stock, 0);
  /** Shelf currently being edited in product picker modal */
  const pickShelf = pickFor != null ? s.shelves.find((sh) => sh.id === pickFor) : null;
  /** Count of shelves without assigned products */
  const emptyShelves = s.shelves.filter((sh) => !sh.itemId).length;
  /** Set of product IDs already placed on shelves */
  const usedIds = new Set(s.shelves.filter((sh) => sh.itemId).map((sh) => sh.itemId));
  /** Count of unlocked products not yet assigned to any shelf */
  const unusedProducts = s.unlocked.filter((id) => !usedIds.has(id)).length;

  return (
    <section className="panel p-4">
      {/* Header with shelf count, auto-stock button, and event/low stock warnings */}
      <div className="flex flex-wrap items-center gap-2 mb-3">
        ...
        {/* Auto-place products on empty shelves */}
        <button
        ...
        {/* Event banner and low stock warning */}
        <div className="ml-auto flex flex-col items-end gap-1 max-w-[60%]">
          ...
          {/* Warning when total stock is critically low */}
          {totalStock < 10 && (
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 xl:grid-cols-4 gap-3">
        {Array.from({ length: MAX_SLOTS }).map((_, slot) => {
          ...
          // Slot beyond leased space - offer to lease more
      ...
      {/* product picker modal - overlay for selecting which product to assign to a shelf */}
      {pickShelf && (
        <div className="fixed inset-0 z-50 bg-ink/50 flex items-center justify-center p-4" onClick={() => setPickFor(null)}>
          <div className="panel p-5 max-w-md w-full anim-pop" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="flex flex-col gap-2 max-h-[50vh] overflow-y-auto pr-1">
              ...
              {/* Locked products - shown but disabled */}
            ...
            {/* Clear shelf button if product already assigned */}
            {pickShelf.itemId && (
              <button
